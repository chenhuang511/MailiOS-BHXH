...
#include "../botan/gnump_engine.h"
#endif

#if defined(BOTAN_HAS_ENGINE_OPENSSL)
#include "../botan/openssl_engine.h"
#endif
...
Allocator* Library_State::get_allocator(const std::string& type) const {
	...
	if (!cached_default_allocator) {
		cached_default_allocator = search_map<std::string, Allocator*> (
				alloc_factory, default_allocator_name, 0);
	}
...
std::string Library_State::get(const std::string& section,
	...
	return search_map<std::string, std::string> (config, section + "/" + key,
			"");
}
 ...
 */
bool Library_State::is_set(const std::string& section, const std::string& key) const {
	Mutex_Holder lock(config_lock);
...
RandomNumberGenerator& Library_State::global_rng() {
	...
	if (!global_rng_ptr)
		global_rng_ptr = make_global_rng(algorithm_factory(), global_rng_lock);
