...
BigInt::BigInt(const BigInt& b) {
	...
	if (b_words) {
		...
		reg1.resize(round_up<size_t> (b_words, 8));
		reg1.copy(b.data(), b_words);
		set_sign(b.sign());
	} else {
		reg.resize(2);
		reg1.resize(2);
...
BigInt::BigInt(const std::string& str) {
	...
	if (str.length() > markers + 2 && str[markers] == '0' && str[markers + 1]
			== 'x') {
		markers += 2;
		base = Hexadecimal;
...
s32bit BigInt::cmp(const BigInt& n, bool check_signs) const {
	if (check_signs) {
		if (n.is_positive() && this->is_negative())
			return -1;
		if (n.is_negative() && this->is_positive())
			return 1;
 ...
 * Hoang them de test
 */
byte BigInt::byte_at_1(size_t n) const {
	//	const size_t WORD_BYTES = sizeof(word);
	const size_t WORD_BYTES = 4;
	size_t word_num = n / WORD_BYTES, byte_num = n % WORD_BYTES;
	if (word_num >= size())
		...
		//		//LOGI("%d",get_reg()[word_num]);
		byte re = get_byte(WORD_BYTES - byte_num - 1, get_reg()[word_num]);
		return re;
	}
}

size_t BigInt::sig_words() const {
	//	//LOGI("vao vao");
	const word* x = &reg[0];
	size_t sig = reg.size();
 ...
 * Hoang them vao
 */
size_t BigInt::bytes_1() const {
	return (bits_1() + 7) / 8;
}

size_t BigInt::bits_1() const {
	//	const size_t words = sig_words();
	const size_t words = 24;
	if (words == 0)
		return 0;
	//LOGI("12");
 ...
 * Hoang them de test
 */
//void BigInt::binary_encode_1(byte output[]) const {
//	const size_t sig_bytes = bytes();
//	//LOGI("encode - %d - %d", sizeof(output), sig_bytes);
//	for (size_t i = 0; i != sig_bytes; ++i) {
//		output[sig_bytes - i - 1] = byte_at_1(i);
//	}
//
//}
