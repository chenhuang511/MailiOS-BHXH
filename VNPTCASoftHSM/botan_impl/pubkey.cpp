 ...
 * Encrypt a message
 */
SecureVector<byte> PK_Encryptor_EME::enc(const byte in[], size_t length,
		RandomNumberGenerator& rng) const {
	if (eme) {
		SecureVector<byte> encoded = eme->encode(in, length,
				op->max_input_bits(), rng);

		if (8 * (encoded.size() - 1) + high_bit(encoded[0])
				> op->max_input_bits())
 ...
 * Decrypt a message
 */
SecureVector<byte> PK_Decryptor_EME::dec(const byte msg[], size_t length) const {
	try {
 ...
 */
PK_Signer::PK_Signer(const Private_Key& key, const std::string& emsa_name,
		Signature_Format format, Fault_Protection prot) {
	Algorithm_Factory::Engine_Iterator i(global_state().algorithm_factory());
...
bool PK_Signer::self_test_signature(const MemoryRegion<byte>& msg,
	...
	if (verify_op->with_recovery()) {
		...
		//LOGI("2.2 - %d - %d", msg.size(),recovered.size());
		...
		//LOGI("2.6");
	return verify_op->verify(&msg[0], msg.size(), &sig[0], sig.size());
}
...
SecureVector<byte> PK_Signer::signature(RandomNumberGenerator& rng) {
	...
	bool check = self_test_signature(encoded, plain_sig);
	//LOGI("3");
	...
	if (sig_format == DER_SEQUENCE) {
		...
		for (size_t j = 0; j != sig_parts.size(); ++j)
			sig_parts[j].binary_decode(&plain_sig[SIZE_OF_PART * j],
					SIZE_OF_PART);

		return DER_Encoder() .start_cons(SEQUENCE) .encode_list(sig_parts) .end_cons() .get_contents();
	} else
 ...
 */
PK_Verifier::PK_Verifier(const Public_Key& key, const std::string& emsa_name,
	...
	try {
		if (sig_format == IEEE_1363)
			...
			while (ber_sig.more_items()) {
				...
				real_sig += BigInt::encode_1363(sig_part,
			...
			return validate_signature(emsa->raw_data(), &real_sig[0],
					real_sig.size());
		} else
...
bool PK_Verifier::validate_signature(const MemoryRegion<byte>& msg,
	...
	if (op->with_recovery()) {
		...
		SecureVector<byte> encoded = emsa->encoding_of(msg,
				op->max_input_bits(), rng);
...
SymmetricKey PK_Key_Agreement::derive_key(size_t key_len, const byte in[],
		size_t in_len, const byte params[], size_t params_len) const {
