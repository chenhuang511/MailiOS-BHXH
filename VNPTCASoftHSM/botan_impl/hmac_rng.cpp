...
void hmac_prf(MessageAuthenticationCode* prf, MemoryRegion<byte>& K,
		u32bit& counter, const std::string& label) {
	prf->update(K);
	prf->update(label);
...
HMAC_RNG::HMAC_RNG(MessageAuthenticationCode* extractor_mac,
		MessageAuthenticationCode* prf_mac) : extractor(extractor_mac), prf(prf_mac) {
	if (!prf->valid_keylength(64)
			|| !extractor->valid_keylength(32))
		throw Invalid_Argument("HMAC_RNG: Bad algo combination "
				+ extractor->name() + " and " + prf->name());
...
HMAC_RNG::~HMAC_RNG() {
	...
	std::for_each(entropy_sources.begin(), entropy_sources.end(), del_fun<
			EntropySource> ());
