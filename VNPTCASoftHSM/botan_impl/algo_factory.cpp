...
//
//#define  LOG_TAG    "algo_factory.cpp"
 ...
 */
template<typename T>
T* engine_get_algo(Engine*, const SCAN_Name&, Algorithm_Factory&) {
	return 0;
}
...
MessageAuthenticationCode* engine_get_algo(Engine* engine,
		const SCAN_Name& request, Algorithm_Factory& af) {
	return engine->find_mac(request, af);
}

template<>
PBKDF* engine_get_algo(Engine* engine, const SCAN_Name& request,
		Algorithm_Factory& af) {
...
const T* factory_prototype(const std::string& algo_spec,
		const std::string& provider, const std::vector<Engine*>& engines,
		Algorithm_Factory& af, Algorithm_Cache<T>* cache) {
	if (const T* cache_hit = cache->get(algo_spec, provider))
		return cache_hit;
	//	LOGI("1- %d", engines.size());
 ...
 */
std::vector<std::string> Algorithm_Factory::providers_of(
...
Algorithm_Factory::prototype_block_cipher(const std::string& algo_spec,
		const std::string& provider) {
	return factory_prototype<BlockCipher> (algo_spec, provider, engines, *this,
			block_cipher_cache);
}
...
Algorithm_Factory::prototype_pbkdf(const std::string& algo_spec,
		const std::string& provider) {
	return factory_prototype<PBKDF> (algo_spec, provider, engines, *this,
			pbkdf_cache);
}
...
Algorithm_Factory::make_mac(const std::string& algo_spec,
		const std::string& provider) {
	//	MessageAuthenticationCode* mac = 0;
	//	LOGI("state 1");
	const MessageAuthenticationCode* proto = prototype_mac(algo_spec, provider);
	//	LOGI("state 2");
	if (proto)
		return proto->clone();
	throw Algorithm_Not_Found(algo_spec);
 ...
 * Add a new PBKDF
 */
void Algorithm_Factory::add_pbkdf(PBKDF* pbkdf, const std::string& provider) {
