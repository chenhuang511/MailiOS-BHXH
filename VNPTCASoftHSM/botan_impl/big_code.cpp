...
void BigInt::encode(byte output[], const BigInt& n, Base base) {
	if (base == Binary) {
//		SecureVector<word> temp = n.get_reg();
//		//LOGI("big_code->encode  -- %d", n.get_reg()[0]);
		...
		hex_encode(reinterpret_cast<char*> (output), &binary[0], binary.size());
	} else if (base == Octal) {
		BigInt copy = n;
		const size_t output_size = n.encoded_size(Octal);
		for (size_t j = 0; j != output_size; ++j) {
			output[output_size - 1 - j] = Charset::digit2char(
					static_cast<byte> (copy % 8));
		...
		for (size_t j = 0; j != output_size; ++j) {
			...
			output[output_size - 1 - j] = Charset::digit2char(
					static_cast<byte> (remainder.word_at(0)));
			if (copy.is_zero())
				break;
		}
	} else
...
BigInt BigInt::decode(const byte buf[], size_t length, Base base) {
	...
	if (base == Binary)
		...
		if (length % 2) {
            ...
            const  char buf0_with_leading_0[2] = { '0', '1' };
			binary = hex_decode(buf0_with_leading_0, 2);

			binary += hex_decode(reinterpret_cast<const char*> (&buf[1]),
					length - 1, false);
		} else
			binary = hex_decode(reinterpret_cast<const char*> (buf), length,
					false);
