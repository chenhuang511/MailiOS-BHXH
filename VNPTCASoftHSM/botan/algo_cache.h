...
public:
	 ...
	 */
	const T
			* get(const std::string& algo_spec,
	 ...
	 */
	void add(T* algo, const std::string& requested_name,
 ...
 */
template<typename T>
typename Algorithm_Cache<T>::algorithms_iterator Algorithm_Cache<T>::find_algorithm(
 ...
 */
template<typename T>
void Algorithm_Cache<T>::add(T* algo, const std::string& requested_name,
		const std::string& provider) {
	if (!algo)
		return;
	...
	if (algo->name() != requested_name && aliases.find(requested_name)
			== aliases.end()) {
 ...
 */
template<typename T> std::vector<std::string> Algorithm_Cache<T>::providers_of(
