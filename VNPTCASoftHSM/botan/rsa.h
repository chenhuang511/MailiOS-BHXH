 ...
 * RSA Public Key
 */
class RSA_PublicKey: public virtual IF_Scheme_PublicKey {
public:
 ...
 * RSA Private Key
 */
class RSA_PrivateKey: public RSA_PublicKey, public IF_Scheme_PrivateKey {
public:
	AlgorithmIdentifier m_AI;
	MemoryVector<byte> m_bytevector;
	bool check_key(RandomNumberGenerator& rng, bool) const;
	/*
	 *  Inherited pure virtual algo_name
	 */

	virtual std::string algo_name() const
	{
		return RSA_PublicKey::algo_name();
	}
	...
	virtual AlgorithmIdentifier algorithm_identifier() const
	{
	 ...
	 virtual MemoryVector<byte> x509_subject_public_key() const
	{
//
		 return Botan::IF_Scheme_PublicKey::x509_subject_public_key();

	}
	RSA_PrivateKey(const AlgorithmIdentifier& alg_id,
			const MemoryRegion<byte>& key_bits, RandomNumberGenerator& rng) :
	 ...
	 */
	RSA_PrivateKey(RandomNumberGenerator& rng, const BigInt& p,
			const BigInt& q, const BigInt& e, const BigInt& d = 0,
			const BigInt& n = 0) :
...
public:
	...
	SecureVector<byte> verify_mr(const byte msg[], size_t msg_len) {
		...
		BigInt i = public_op(m);

		return BigInt::encode(i);
	}
