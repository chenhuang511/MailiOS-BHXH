...
public:
	 ...
	 */
	enum Base {
		Octal = 8, Decimal = 10, Hexadecimal = 16, Binary = 256
	};
	...
	struct DivideByZero: public Exception {
		DivideByZero() :
			Exception("BigInt divide by zero") {
		}
	};
	 ...
	 */
	static BigInt random_integer(RandomNumberGenerator& rng, const BigInt& min,
			const BigInt& max);
