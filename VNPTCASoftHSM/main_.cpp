...
CK_FUNCTION_LIST function_list = { { 2, 20 }, C_Initialize, C_Finalize,
		C_GetInfo, C_GetFunctionList, C_GetSlotList, C_GetSlotInfo,
		C_GetTokenInfo, C_GetMechanismList, C_GetMechanismInfo, C_InitToken,
		C_InitPIN, C_SetPIN, C_OpenSession, C_CloseSession, C_CloseAllSessions,
		C_GetSessionInfo, C_GetOperationState, C_SetOperationState, C_Login,
		C_Logout, C_CreateObject, C_CopyObject, C_DestroyObject,
		C_GetObjectSize, C_GetAttributeValue, C_SetAttributeValue,
		C_FindObjectsInit, C_FindObjects, C_FindObjectsFinal, C_EncryptInit,
		C_Encrypt, C_EncryptUpdate, C_EncryptFinal, C_DecryptInit, C_Decrypt,
		C_DecryptUpdate, C_DecryptFinal, C_DigestInit, C_Digest, C_DigestUpdate,
		C_DigestKey, C_DigestFinal, C_SignInit, C_Sign, C_SignUpdate,
		C_SignFinal, C_SignRecoverInit, C_SignRecover, C_VerifyInit, C_Verify,
		...
		C_DecryptVerifyUpdate, C_GenerateKey, C_GenerateKeyPair, C_WrapKey,
...
//	}
	...
	if (!session->pkSigner || session->signMech != pMechanism->mechanism
		...
		case CKM_SHA1_RSA_PKCS_PSS:
			...
			if (CK_RSA_PKCS_PSS_PARAMS_PTR(pMechanism->pParameter)->hashAlg
					!= CKM_SHA_1) {
				//DEBUG_MSG("C_SignInit", "hashAlg must be CKM_SHA_1");
				return CKR_ARGUMENTS_BAD;
			}
			if (CK_RSA_PKCS_PSS_PARAMS_PTR(pMechanism->pParameter)->mgf
					!= CKG_MGF1_SHA1) {
				//DEBUG_MSG("C_SignInit", "mgf must be CKG_MGF1_SHA1");
				return CKR_ARGUMENTS_BAD;
			}
...
//    uPIN = userPin;
	...
	CK_ATTRIBUTE pukAttribs[] = {{CKA_LABEL, pubkeyLabel, sizeof(pubkeyLabel)}, {CKA_CLASS, &opubKey, sizeof(opubKey)}, { CKA_LOCAL, &bTrue, sizeof(bTrue) }, {
			...
			&bPrivatePuk, sizeof(bPrivatePuk) }, { CKA_ID, cid, sizeof(cid) }, { CKA_ENCRYPT, &bFalse,
			sizeof(bFalse) }, { CKA_VERIFY, &bTrue, sizeof(bTrue) }, { CKA_WRAP,
	...
	CK_ATTRIBUTE prkAttribs[] = {{CKA_LABEL, prikeyLabel, sizeof(prikeyLabel)}, {CKA_CLASS, &opriKey, sizeof(opriKey)}, { CKA_LOCAL, &bTrue, sizeof(bTrue) },
			...
			sizeof(subject) }, { CKA_ID, cid, sizeof(cid) }, { CKA_SENSITIVE,
			&bTrue, sizeof(bTrue) }, { CKA_DECRYPT, &bTrue, sizeof(bTrue) }, {
			CKA_SIGN, &bTrue, sizeof(bTrue) }, { CKA_UNWRAP, &bFalse,
			sizeof(bFalse) } };
	hPuk = CK_INVALID_HANDLE;
	hPrk = CK_INVALID_HANDLE;
	CK_RV rv = C_GenerateKeyPair(hSession, &mechanism, pukAttribs,
		...
		CK_UTF8CHAR userPin[], CK_ULONG oPuk, CK_ULONG oPrk, CK_ULONG keyLength,char* id)
{
	...
	rv = C_Login(c_hSession, CKU_USER, userPin, pinLength);
...
//			IS_PRIVATE, oPuk, oPrk, ukeyLength);
   ...
   // memcpy(id, id)
	rv = generateRsaKeyPair(c_hSession, ckFalse, ckFalse, ckFalse,
...
long genPKCS10Request(CK_SESSION_HANDLE sessionID, CK_SLOT_ID slotInitToken,
	...
	rv = C_Login(sessionID, CKU_USER, userPin, pinLength);
...
//	LOGI("PKCS10 OK");
	...
	// Create cert.pem file
	FILE* file = fopen("/Users/TranHa/Documents/request.pem", "w+");
	if (!file) {
		printf("Error when opening file cert.pem");
	} else {
		std::string pemStr = req.PEM_encode();
