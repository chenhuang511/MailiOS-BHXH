...
- (void) _refresh
{
    ...
    if (IDIOM == IPAD) {
        ...
        [html appendFormat:@"<html><head><script src=\"%@\"></script><style type='text/css'>body{ font-family: 'Helvetica Neue', Helvetica, Arial; margin:0; padding:20px;}\
         hr {border: 0; height: 1px; background-color: #bdc3c7;}\
         .show { display: none;}.hide:target + .show { display: inline ;} .hide:target { display: none;} .content { display:none;} .hide:target ~ .content { display:inline;}\
         </style></head><body>%@</body><iframe src='x-mailcore-msgviewloaded:' style='width: 0px; height: 0px; border: none;'></iframe></html>",
         [jsURL absoluteString], content];
    ...
    _msgContent = content;
//  HoangTD edit
//    // Chỉ kích hoạt tự động khi mail thoả mãn 2 điêu kiện: là mail chưa đọc và là mail chứa nội dung thích hợp
//    BOOL flagSeen = [MsgListViewController shareFlagSeen];
//    if (flagSeen && [content rangeOfString:@"Hãy vào ứng dụng TokenManager, điền vào mã kích hoạt để"].location != NSNotFound && [content rangeOfString:@"Mã kích hoạt chứng thư:"].location != NSNotFound) {
//        NSString *prefix = @"Mã kích hoạt chứng thư: <b>";
//        NSString *suffix = @"</b>";
//        
//        NSString *prefixString = [content substringFromIndex:[content rangeOfString:prefix].location + prefix.length];
//        activeCode = [prefixString substringToIndex:[prefixString rangeOfString:suffix].location];
//        NSString *msg = [NSString stringWithFormat:@"Mã kích hoạt chứng thư\n%@\nBạn có muốn kích hoạt chứng thư?", activeCode];
//        
//        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Mã kích hoạt"
//                                                        message:msg
//                                                       delegate:self
...
- (void)alertView:(UIAlertView *)sucess clickedButtonAtIndex:(NSInteger)buttonIndex {
//  HoangTD edit
//    if (buttonIndex == 1){
//        NSLog(@"Call Token");
//        UIApplication *ourApplication = [UIApplication sharedApplication];
//        NSString *urlActive = [NSString stringWithFormat:@"vnptcatokenmanager://?emailcall/%@", activeCode];
//        NSURL *ourURL = [NSURL URLWithString:urlActive];
...
//                                                      withCallbackURL:[NSURL URLWithString: @"mCA Mail://back"]
//                                                         createNewTab:YES];
//            }
