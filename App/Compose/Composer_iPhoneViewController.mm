...
- (void)didSelectItemAtIndex:(NSUInteger)index {
  ...
  if (sign) {
    ...
    if (token == HARDTOKEN) {
      ...
      NSString *passHardToken =
          [[NSUserDefaults standardUserDefaults] stringForKey:@"HardPasswrd"];
      if (!passHardToken) {
        [self.view endEditing:YES];
...
- (void)sendEmailto:(NSArray *)to
  ...
  __block NSString *to_ = [to componentsJoinedByString:@", "];
